        ...
        <>
            ...
            <div className="grid-container">
                {shuffledAnswer.map((answer, idx) => {
                    const bgColor = showAnswer ? answer === correct_answer ? 'text-green' : 'text-red' : 'text-black';
                    return (<button id="quizOption" key={idx} className={`${bgColor}`} onClick={() => handleAnswer(answer)} dangerouslySetInnerHTML={{ __html: answer }} />
                    )
                })}.
